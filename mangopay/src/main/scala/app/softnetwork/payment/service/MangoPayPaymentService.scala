          ...
          case Some(s) =>
            s match {
              case EventType.KYC_FAILED =>
                run(
                  ...
                  case _: KycDocumentStatusUpdated =>
                    logger.info(
                      s"[Payment Hooks] KYC has been updated for $ressourceId -> $eventType"
                    )
                    complete(HttpResponse(StatusCodes.OK))
                  case _ =>
                    logger.warn(
                      s"[Payment Hooks] KYC has not been updated for $ressourceId -> $eventType"
                    )
                    complete(HttpResponse(StatusCodes.OK))
                }
              ...
              case EventType.UBO_DECLARATION_REFUSED =>
                run(
                  ...
                  case UboDeclarationStatusUpdated =>
                    logger.info(
                      s"[Payment Hooks] Ubo Declaration has been updated for $ressourceId -> $eventType"
                    )
                    complete(HttpResponse(StatusCodes.OK))
                  case _ =>
                    logger.warn(
                      s"[Payment Hooks] Ubo Declaration has not been updated for $ressourceId -> $eventType"
                    )
                    complete(HttpResponse(StatusCodes.OK))
                }
              ...
              case EventType.USER_KYC_REGULAR =>
                run(ValidateRegularUser(ressourceId)) completeWith {
                  case RegularUserValidated =>
                    logger.info(
                      s"[Payment Hooks] Regular User has been validated for $ressourceId -> $eventType"
                    )
                    complete(HttpResponse(StatusCodes.OK))
                  case _ =>
                    logger.warn(
                      s"[Payment Hooks] Regular User has not been validated for $ressourceId -> $eventType"
                    )
                    complete(HttpResponse(StatusCodes.OK))
                }
              case EventType.MANDATE_FAILED =>
                run(
                  UpdateMandateStatus(ressourceId, Some(BankAccount.MandateStatus.MANDATE_FAILED))
                ) completeWith {
                  case _: MandateStatusUpdated =>
                    logger.info(
                      s"[Payment Hooks] Mandate has been updated for $ressourceId -> $eventType"
                    )
                    complete(HttpResponse(StatusCodes.OK))
                  case _ =>
                    logger.warn(
                      s"[Payment Hooks] Mandate has not been updated for $ressourceId -> $eventType"
                    )
                    complete(HttpResponse(StatusCodes.OK))
                }
              ...
              case EventType.MANDATE_CREATED =>
                run(
                  UpdateMandateStatus(ressourceId, Some(BankAccount.MandateStatus.MANDATE_CREATED))
                ) completeWith {
                  case _: MandateStatusUpdated =>
                    logger.info(
                      s"[Payment Hooks] Mandate has been updated for $ressourceId -> $eventType"
              ...
              case EventType.MANDATE_EXPIRED =>
                run(
                  UpdateMandateStatus(ressourceId, Some(BankAccount.MandateStatus.MANDATE_EXPIRED))
                ) completeWith {
                  case _: MandateStatusUpdated =>
                    logger.info(
                      s"[Payment Hooks] Mandate has been updated for $ressourceId -> $eventType"
