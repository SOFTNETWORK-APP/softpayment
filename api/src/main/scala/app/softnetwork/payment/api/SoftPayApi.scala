...
trait SoftPayApi[SD <: SessionData with SessionDataDecorator[SD]] extends SoftPayApplication {
  ...
  override def internalAccountEvents2AccountProcessorStream
    ...
    new InternalAccountEvents2AccountProcessorStream
      ...
      override def config: Config = SoftPayApi.this.config
  ...
  def paymentSwagger: ActorSystem[_] => SwaggerEndpoint = sys =>
    new PaymentServiceEndpoints[SD]
      with PaymentHandler
