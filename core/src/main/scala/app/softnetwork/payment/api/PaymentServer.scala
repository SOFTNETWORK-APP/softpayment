...
import app.softnetwork.payment.message.PaymentMessages.{CancelMandate, CancelPreAuthorization, CreateOrUpdatePaymentAccount, DirectDebit, DirectDebited, LoadDirectDebitTransaction, MandateCanceled, PaidIn, PaidOut, PayInWithCardPreAuthorized, PayOut, PaymentAccountCreated, PaymentAccountUpdated, PaymentCommand, PaymentError, PreAuthorizationCanceled, RecurringPaymentRegistered, Refund, Refunded, RegisterRecurringPayment, Transfer, Transfered}
...
trait PaymentServer extends PaymentServiceApi with GenericPaymentHandler {_:  CommandTypeKey[PaymentCommand] =>
  implicit def system: ActorSystem[_]

  implicit lazy val ec: ExecutionContextExecutor = system.executionContext

  override def createOrUpdatePaymentAccount(in: CreateOrUpdatePaymentAccountRequest): Future[CreateOrUpdatePaymentAccountResponse] = {
    in.paymentAccount match {
      case Some(paymentAccount) =>
        !? (CreateOrUpdatePaymentAccount(paymentAccount)) map {
          case PaymentAccountCreated | PaymentAccountUpdated => CreateOrUpdatePaymentAccountResponse(true)
  ...
  override def payInWithCardPreAuthorized(in: PayInWithCardPreAuthorizedRequest): Future[TransactionResponse] = {
    import in._
  ...
  override def cancelPreAuthorization(in: CancelPreAuthorizationRequest): Future[CancelPreAuthorizationResponse] = {
    import in._
    !? (CancelPreAuthorization(orderUuid, cardPreAuthorizedTransactionId)) map {
      case r: PreAuthorizationCanceled => CancelPreAuthorizationResponse(Some(r.preAuthorizationCanceled))
  ...
  override def transfer(in: TransferRequest): Future[TransferResponse] = {
    import in._
    !? (Transfer(orderUuid, debitedAccount, creditedAccount, debitedAmount, feesAmount, currency, payOutRequired, externalReference)) map {
      case r: Transfered => TransferResponse(Some(r.transferedTransactionId), r.paidOutTransactionId)
  ...
  override def directDebit(in: DirectDebitRequest): Future[TransactionResponse] = {
    import in._
    !? (DirectDebit(creditedAccount, debitedAmount, feesAmount, currency, statementDescriptor, externalReference)) map {
  ...
  override def loadDirectDebitTransaction(in: LoadDirectDebitTransactionRequest): Future[TransactionResponse] = {
    import in._
  ...
  override def registerRecurringPayment(in: RegisterRecurringPaymentRequest): Future[RegisterRecurringPaymentResponse] = {
    import in._
    ...
    maybeType match {
      case Some(atype) =>
        !? (
          ...
          case r: RecurringPaymentRegistered => RegisterRecurringPaymentResponse(Some(r.recurringPaymentRegistrationId))
