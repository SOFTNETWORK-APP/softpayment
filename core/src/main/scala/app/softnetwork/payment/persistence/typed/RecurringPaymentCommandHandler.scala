                      ...
                      case Some(walletId) =>
                        cmd.cardId
                          ...
                          case Some(cardId) =>
                            ...
                            val attachmentResult: Either[PaymentResult, Unit] =
                              loadPaymentMethod(cardId) match {
                                ...
                                case Some(card: Card) if card.customerId.exists(_ != userId) =>
                                  ...
                                  Left(CardNotAttachedToCustomer)
                                case None =>
                                  log.warn(
                            ...
                            attachmentResult match {
                              case Left(error) =>
                                Effect.none.thenRun(_ => error ~> replyTo)
                              case Right(_) =>
                                registerRecurringCardPayment(
                                  ...
                                  case _ =>
                                    Effect.none
                                      .thenRun(_ => RecurringPaymentNotRegistered ~> replyTo)
                                }
                            }
                ...
                if (first) {
                  FirstRecurringPaidInEvent.defaultInstance
                    ...
                    .copy(
                      ...
                      correlationId = correlationId // Story 13.7 — durable hop
                    )
                } else {
